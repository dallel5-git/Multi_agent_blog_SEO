...
identifiants Instagram (`PILOTAGE_INSTAGRAM_BOT_TOKEN` / `_CHAT_ID`).
"""
...
def create_bot(settings: PilotageSettings, repository: CalendarRepository) -> PilotageBot:
    creds = settings.bots.for_platform(Platform.INSTAGRAM)
    return create_bot_for_platform(
        Platform.INSTAGRAM,
        token=creds.bot_token,
        chat_id=creds.chat_id,
        offset_path=settings.calendar.db_path.parent / "bot_offsets" / "instagram.json",
